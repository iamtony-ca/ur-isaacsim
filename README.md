# UR16e — ROS 2 Jazzy + Isaac Sim 6.0.1 (sim & real 공용 제어 스택 + dual-tool CAD 파이프라인)
...
`ur_bringup` 한 패키지에 모여 있다. `ur_ws` 를 복제해 그 위에 **세트4 = dual-tool EOAT 의
CAD(STEP)→Isaac Sim 자동화 파이프라인**을 새로 얹었다(아래 §2, `STEP_TO_SIM.md`).

- 워크스페이스: `/isaac-sim/volume/ur_dualtool_ws` (colcon), 코드 트리 = `src/`
- 환경: Docker, ROS 2 **Jazzy**, **Isaac Sim 6.0.1**(`/isaac-sim`, 실측 6.0.1-rc.7), GPU
  - 세트1~3 제어 스택은 `ur_ws` 에서 5.1.0 으로 개발됐고 이 워크스페이스에서 6.0.1 로 재검증됨
    (HISTORY.md 의 과거 로그엔 5.1.0/`ur_ws` 표기가 이력 그대로 남아 있음).
...
| **4. dual-tool EOAT** | `ur16e_dualtool.launch.py` (+`…_moveit`) | (실물 미연결) | OnRobot Dual QC 로 **2FG14 그리퍼 + ESTIC 스크류드라이버(+Copick3D ×1) 동시 장착**. STEP(CAD)→USD→단일 아티큘레이션 파이프라인. **live 검증됨(2026-07-17)**: 충돌탐색+plan/execute+MoveJ/MoveL, RViz==Isaac(mesh URDF), **정적 장애물 충돌 파이프라인** 포함 |
...
> 세트3 `ur16e_2f85_d405_moveit`(depth→OctoMap, `use_octomap` 기본 on), 세트4 `ur16e_dualtool_moveit`.
> **세트4 는 CAD 자동화 파이프라인이 핵심** — 저작 절차·config 스키마는 [`STEP_TO_SIM.md`](STEP_TO_SIM.md)
> 와 `ur_bringup/isaac/common/eoat/README.md`, 재현은 [`SETUP.md`](SETUP.md) §5-D.
...
│   └── ur16e_dualtool/   ur16e_dualtool[_moveit].launch.py                      (세트4)
├── config/
...
├── urdf/ur16e_dualtool/  ur16e_dualtool_sim.urdf.xacro + …_eoat_macro.xacro(생성, mesh 참조)   (세트4)
├── srdf/common/          … + ur16e_dualtool.srdf.xacro(생성)                         (세트4)
└── meshes/               PickNik 브라켓 메시 (세트3) + eoat/(세트4 부품 mesh, export_eoat_meshes 생성)
```
...
> 매 터미널 먼저: `source /opt/ros/jazzy/setup.bash && source /isaac-sim/volume/ur_dualtool_ws/install/setup.bash && export ROS_DOMAIN_ID=0`
...
/isaac-sim/python.sh src/ur_bringup/isaac/common/ur16e_isaac_ros2.py \
    --asset-path /isaac-sim/volume/ur_dualtool_ws/src/ur_bringup/isaac/assets/ur16e_2f85_d405.usd \
