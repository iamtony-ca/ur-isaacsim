...
# --- (PLAYGROUND) publish the eye-in-hand camera pose on /tf from Isaac itself ---
parser.add_argument("--camera-tf", action="store_true",
                    help="also publish the eye-in-hand camera pose on /tf via a ROS2PublishTransformTree "
                         "node (Isaac-side TF, instead of relying on the ROS URDF robot_state_publisher). "
                         "Only takes effect together with --with-camera.")
parser.add_argument("--camera-tf-parent", default="base_link",
                    help="prim (under the robot) used as the TF parent frame for the camera; "
                         "empty string ('') = World. The published child frame is the camera prim name.")
...
if args.with_camera:
    ...
    # ---- (PLAYGROUND) camera TF publisher --------------------------------
    # Add a ROS2PublishTransformTree node to the EXISTING /CameraGraph in a
    # SECOND og.Controller.edit call. It reuses the graph's OnTick + CamContext
    # (referenced by name) and adds its own ReadSimTime for the header stamp, so
    # Isaac itself broadcasts base_link -> camera on /tf every rendered step --
    # the same TF the ROS-side robot_state_publisher would otherwise supply.
    #
    # NOTE on frame names / orientation (important for the real perception stack):
    #  * The node names frames by PRIM NAME: child = "d405_camera" (the camera
    #    prim), parent = the parent prim's name (e.g. "base_link"). These are NOT
    #    the optical-frame names the image topics use (camera_depth_optical_frame),
    #    so this TF is for learning/visualisation, not a drop-in for the perception
    #    stack. To match, rename/parent an optical-frame prim, or use
    #    ROS2PublishRawTransformTree with explicit frame strings.
    #  * The pose broadcast is the USD camera pose (looks down -Z), i.e. USD-camera
    #    convention, not ROS optical (z-forward). Same reason to treat it as a demo.
    if args.camera_tf:
        _tf_parent = args.camera_tf_parent.strip()
        _parent_path = f"{ROBOT_PRIM}/{_tf_parent}" if _tf_parent else None
        try:
            _keys = og.Controller.Keys
            _tf_edits = {
                _keys.CREATE_NODES: [
                    ("CamReadSimTime", "isaacsim.core.nodes.IsaacReadSimulationTime"),
                    ("CameraTF", "isaacsim.ros2.bridge.ROS2PublishTransformTree"),
                ],
                _keys.CONNECT: [
                    # Pre-existing nodes must be referenced by FULL graph path in a
                    # follow-up edit; only nodes created in THIS edit (CameraTF,
                    # CamReadSimTime) resolve by short name.
                    ("/CameraGraph/OnTick.outputs:tick", "CameraTF.inputs:execIn"),
                    ("/CameraGraph/CamContext.outputs:context", "CameraTF.inputs:context"),
                    ("CamReadSimTime.outputs:simulationTime", "CameraTF.inputs:timeStamp"),
                ],
                _keys.SET_VALUES: [
                    ("CameraTF.inputs:topicName", "/tf"),
                    ("CameraTF.inputs:targetPrims", [usdrt.Sdf.Path(CAM_PRIM)]),
                ],
            }
            if _parent_path is not None:
                _tf_edits[_keys.SET_VALUES].append(
                    ("CameraTF.inputs:parentPrim", [usdrt.Sdf.Path(_parent_path)]))
            # editing an EXISTING graph: pass the path STRING, not the
            # {graph_path, evaluator_name} dict (the dict form CREATES a graph and
            # errors with "a graph already exists at this path").
            og.Controller.edit("/CameraGraph", _tf_edits)
            print(f"  camera TF (/tf)     : parent={_parent_path or 'World'}  ->  child={CAM_PRIM}")
        except Exception as e:
            carb.log_error(f"Failed to add camera TF publisher: {e}")
