...
>
> 부호/오프셋/속도상한/clamp 는 **전부 ROS 파라미터**다. 실물 튜닝은 파라미터로만 한다.
> `sign [1,−1,−1,−1,1,−1]` / `offset [180,−90,0,−90,0,0]°` 기본값은 **2026-09-17 실물 L100 실측**(`HISTORY.md` §49.8) —
> URDF 유도값(J5 −1)은 틀렸었다. 다른 개체면 `CHECKLIST.md` E-2 순서(방향 → 오프셋)로 재측정.
...
**★ 시작 자세는 랑데부에서 맞춘다 (2026-09-10; 자세 재정의 2026-09-16 §47 → 실측 확정 2026-09-17 `HISTORY.md` §49.8)** —
임의 자세에서 engage 하지 않는다. 실물 L100 이 손 떼도 서 있는 자세(실측)가 실측 매핑을 통과하면 정확히 `ready` 가 된다:

```
leader [−1.1°, −88.4°, +152.0°, −69.6°, +87.5°, +1.6°]  →  UR16e [178.9°, −1.6°, −152.0°, −20.4°, 87.5°, −1.6°] = reset_pose.py ready
```
(`ready_v2` = 2026-09-16 의 URDF 가지 [0, −180°, +152°, −152°, −90°, 0](같은 실루엣, 손목 오프셋 반대편; sim 스모크만),
`ready_v1` = 그 전 SRDF `home` 매핑 [0, −90°, +90°, −90°, −90°, 0] — 2026-09-16 이전 데이터셋의 시작 자세.)
