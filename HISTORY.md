...
### OnRobot 실물 드라이버 조사 · 경로 재정리 · 참고 구현 확보 (2026-07-21)
- **경로 재정리(A/B/C)**: **A = OnRobot URCap 단독 = PolyScope 방식**(URCap 이 그리퍼 직접 제어, ROS/Isaac 밖) → sim 비호환·금지. **B = tool I/O RS-485**(펜던트엔 ToolComm Forwarder = *dumb 시리얼 통과* URCap 만; 그리퍼 제어는 PC/ros2_control). **C = Compute Box Modbus TCP :502**(이더넷). **★ 핵심 교정**: "B 는 sim 안 됨"은 오해 — **B·C 는 둘 다 실물 백엔드라 sim 에선 topic_based 로 스왑**됨(sim 호환은 경로가 아니라 아키텍처가 보장). sim 비호환은 A 뿐.
- **B vs C 는 sim 이 아니라 배선/센서 문제**: 그리퍼 1개면 **B 가 배선 간단**(tool I/O 끝, 박스 불필요), **HEX F/T 센서/F/T 데이터** 쓰면 C(Compute Box 허브). SW(레지스터맵·GripperCommand)는 B/C 거의 동일 → 부품 후 결정. URCap 충돌 버그는 Robotiq URCap 공존 시만(우리는 그리퍼별 UR 분리라 무관).
- **★ 참고 구현 확보(tonydle, MIT, `src/` clone)**: `OnRobot_ROS2_Driver` = **ROS2 `ros2_control` ActuatorInterface 플러그인**(RG2/6), `IModbusConnection`→**TCP(C)/Serial(B) 통일**(Mazurel/Modbus C++ + libnet), `RG` 클래스 레지스터맵 격리, `finger_width` 단일 조인트, `use_fake_hardware`→mock, 생명주기 완비. 형제 `UR_OnRobot_ROS2`(팔+그리퍼 ROS2) · `ur_onrobot`(ROS1, 경로 B/C bringup 실증). → **§9 를 "새로 짜기"에서 "이식+레지스터 교체"로 낮춤**.
- **★ sim 그리퍼 무변경 분석**(실측): 현 sim = `gripper_2fg14__finger_left_joint`(구동)+`_right_joint`(`<mimic>`), `<hardware>`=`JointStateTopicSystem`(구동핑거 1개만 Isaac 교환), `gripper_controller`=**`GripperActionController`(GripperCommand, max_effort 140)**. → **레퍼런스와 동일 패턴, 계약 이미 정렬.** B/C 실물화 시 **URDF·조인트명·GripperCommand·컨트롤러·SRDF·충돌도구 전부 무변경**, 바뀌는 건 **①`<hardware>` real xacro 분기 ②플러그인 내부 width↔single-finger 스케일(URDF 아님) ③실물 launch** 뿐. (OnRobot 은 "폭" 1값 노출 ↔ 우리는 한쪽 핑거 구동 → 플러그인이 `width=2×핑거(+offset)` 환산.)
- **포팅 계획**: `RG`→`TwoFG14`(레지스터맵 교체=핵심 미지수) · `finger_width` 단일 cmd+URDF `<mimic>` · `JointGroupPositionController`→`GripperActionController` 추가(저자 README TODO) · `<hardware>` real 자리 삽입. blocked = 2FG14 레지스터맵 + Compute Box IP/사양(부품 대기).
- **CAD 개념 Q&A 정리**(`qna.md` **Q8**): 핑거 좌/우 분할 이유(sim 관절=독립 rigid body, 융합 금지)·핑거 원점(자기 장착면, 바디중심 공유 ✗)·collider(`UsdPhysics.CollisionAPI` 근사, Isaac PhysX vs MoveIt 두 충돌세계)·GUI import orientation 한계(up-axis 만, 원점은 `normalize_part.py` 로 bake).
- **문서 갱신**: `quick_start.md` 전면 업데이트(Step5 config 를 실제 `tcp_pose`+2FG14 핑거 구조로, Step11-(e) 파지 휠 충돌체크 신규, verify_articulation 인자, 함정표 is_diff/유령휠). `HARDWARE.md §6`(경로표 재작성·A vs B·B vs C·참고 구현+포팅). `to_do.md §9`(이식+레지스터 교체로 구체화). `README.md`(§3 manip/·§7 세트4). 메모리 `gripper-sim-real-parity`.
