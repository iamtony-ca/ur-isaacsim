...
def mode_match(n, sec, sign):
    ...
    print("\n  ── 적용 (임시) — 브리지가 DISABLED 일 때 바로 먹는다 ────")
    print(f'  ros2 param set /omy_to_ur16e offset "[{lst}]"')
    print("  · ENGAGED 중이면 거부된다(매핑이 바뀌면 팔이 움직인다) → 먼저 /omy_bridge/disable")
    print("  · 적용 즉시 /omy_bridge/engage_error 에 반영되므로 보면서 조작감을 확인한다")
    print("  · offset/sign 만 런타임 변경 가능. 나머지 파라미터는 생성자에서 1회만 읽으므로")
    print("    param set 이 '조용히 무시' 되지 않고 거부되며 재기동 명령을 알려준다.")
    print("\n  ── 영구화 (이게 진짜 할 일) ─────────────────────────────")
