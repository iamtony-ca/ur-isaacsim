...
for i in $(seq 1 "$N"); do
  ...
  sleep 2
  # Same three guards as groot_rollout.sh (HISTORY.md 44.2): a failed streaming
  # switch, a controller that is not ACTIVE, or stale observations all read as
  # "the policy did nothing" and were scored FAIL before. Abort / SKIP instead.
  switched=0
  for a in 1 2 3; do
    if python3 "$WS/src/ur_bringup/isaac/common/switch_control_mode.py" streaming > "$LOG/rn_switch_$i.log" 2>&1; then
      switched=1; break
    fi
    echo "     switch attempt $a: $(tail -1 "$LOG/rn_switch_$i.log")"; sleep 3
  done
  if [ "$switched" != 1 ]; then
    echo "FAIL: could not switch to streaming control:"; sed 's/^/       /' "$LOG/rn_switch_$i.log"; exit 1
  fi
  if ! timeout 10 ros2 control list_controllers 2>/dev/null | awk '$1=="forward_position_controller"{print $NF}' | grep -qx "active"; then
    echo "FAIL: forward_position_controller is not ACTIVE after the switch (policy_inference.launch.py missing?)"; exit 1
  fi
  if ! python3 "$S/obs_ready.py" --cameras "wrist=/camera/color/image_raw"; then
    echo "     SKIP: observations not flowing (see above)"; continue
  fi
  ...
  if [ "$chunks" -le 0 ]; then
    ...
    grep -oE "Error in StreamActions: .*" "$LOG/rn_server.log" 2>/dev/null | sort -u | head -1 | cut -c1-160 | sed 's/^/            /'
    grep -oE "Error in observation sender: .*" "$LOG/rn_client_$i.log" 2>/dev/null | sort -u | head -1 | sed 's/^/            /'
    continue
  fi
  ...
  echo "     start=$pos  $res   (${chunks} chunks served)"
