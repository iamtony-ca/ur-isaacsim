...
rm -rf "$WS/outputs/act_red_left_100"
for W in 4 2 0; do
  echo "== num_workers=$W"
  UR_WS_TORCH_SHM_FIX=1 deps/.venv-ml/bin/lerobot-train \
    --policy.type=act --policy.push_to_hub=false \
    --dataset.repo_id=tony/ur16e_pick_place_red_left_100 --dataset.root="$DS" \
    --steps=60000 --batch_size=32 --num_workers="$W" \
    --output_dir="$WS/outputs/act_red_left_100" > "$LOG/train_100_w$W.log" 2>&1
  if find "$WS/outputs/act_red_left_100" -path "*pretrained_model/config.json" 2>/dev/null | grep -q .; then
    echo "   DONE (workers=$W)"
    grep -oE "[0-9]+/60000 \[[0-9:]+<[0-9:]+, *[0-9.]+s/step\]" "$LOG/train_100_w$W.log" | tail -1
    grep -oE "loss:[0-9.]+" "$LOG/train_100_w$W.log" | tail -1
    break
  fi
  echo "   failed: $(grep -oE '(RuntimeError|OSError|ValueError).*' "$LOG/train_100_w$W.log" | tail -1 | cut -c1-70)"
  rm -rf "$WS/outputs/act_red_left_100"
done
