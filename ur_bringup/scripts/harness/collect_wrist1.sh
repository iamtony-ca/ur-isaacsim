...
ME=$$
N=${1:-50}
OUT="$WS/outputs/il_raw_wrist_only"
...
nohup /isaac-sim/python.sh "$WS/src/ur_bringup/isaac/common/ur16e_isaac_ros2.py" \
  ...
  --object-spacing 0.20 --randomize-object --randomize-radius 0.06 --seed 111 \
  --camera-res 320x240 --grasp-attach --with-camera --with-static-cam --headless \
  > "$LOG/w1_isaac.log" 2>&1 &
for _ in $(seq 1 90); do grep -q "scene topics" "$LOG/w1_isaac.log" && break; sleep 5; done
grep -q "scene topics" "$LOG/w1_isaac.log" || { echo "FAIL: isaac"; exit 1; }
"$S/bringup.sh" > "$LOG/w1_bringup.log" 2>&1
tail -1 "$LOG/w1_bringup.log" | grep -q "== ready" || { echo "FAIL: bringup"; tail -3 "$LOG/w1_bringup.log"; exit 1; }
echo "   stack ready"

rm -rf "$LOG/grasp_w1"; mkdir -p "$LOG/grasp_w1"
nohup python3 "$S/grasp_watch.py" "$LOG/grasp_w1" 14400 0.45 > "$LOG/grasp_w1.log" 2>&1 &
W=$!
TASKS="red|left|put the red block on the left marker" CAM_EXTERIOR=none LOGS="$LOG/w1_logs" \
  "$WS/src/ur_bringup/scripts/collect_il_episodes.sh" "$N" "$OUT"
kill "$W" 2>/dev/null
echo "== grasp frames: $(ls -1 "$LOG/grasp_w1" | wc -l)"
