...
S="$(dirname "$0")"
RAW="$WS/outputs/il_raw_3task_240"
OUT="$WS/outputs/lerobot_ds_240"

for _ in $(seq 1 240); do
  grep -qE "ALL TASKS DONE|FAIL" "$LOG/collect240.log" 2>/dev/null && break
  sleep 15
done
if ! grep -q "ALL TASKS DONE" "$LOG/collect240.log" 2>/dev/null; then
  echo "collection did not finish cleanly:"; tail -3 "$LOG/collect240.log"; exit 1
fi
...
rm -rf "$OUT"
"$WS/deps/.venv-ml/bin/python" "$WS/src/ur_bringup/scripts/raw_to_lerobot.py" \
  --raw "$RAW" --repo-id tony/ur16e_pick_place_3task_240 --root "$OUT" \
  > "$LOG/convert240_run.log" 2>&1
...
PY
  ...
  echo "== CONVERSION FAILED"; tail -5 "$LOG/convert240_run.log"
fi
