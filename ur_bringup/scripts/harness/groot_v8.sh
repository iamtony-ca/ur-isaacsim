...
echo "== isaac (3-task scene, 320x240, static cam)"
nohup /isaac-sim/python.sh "$WS/src/ur_bringup/isaac/common/ur16e_isaac_ros2.py" \
  ...
  > "$LOG/gr8_isaac.log" 2>&1 &
for _ in $(seq 1 90); do grep -q "scene topics" "$LOG/gr8_isaac.log" && break; sleep 5; done
grep -q "scene topics" "$LOG/gr8_isaac.log" || { echo "FAIL: isaac"; tail -5 "$LOG/gr8_isaac.log"; exit 1; }
grep -E "eye-in-hand camera|static cam  " "$LOG/gr8_isaac.log"

echo "== control stack"
"$S/bringup.sh" > "$LOG/gr8_bringup.log" 2>&1
tail -1 "$LOG/gr8_bringup.log" | grep -q "== ready" || {
  echo "FAIL: bringup"; tail -8 "$LOG/gr8_bringup.log"; exit 1; }
echo "   stack ready"
