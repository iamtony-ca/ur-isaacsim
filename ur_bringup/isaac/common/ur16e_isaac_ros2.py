...
# --- KNOWN static obstacles (sim) : from obstacles/obstacles.yaml, the SAME config
#     that feeds the MoveIt planning scene (load_obstacles_moveit.py). Adds each as a
#     static collider at its pose so the Isaac scene matches the collision world. ---
...
# colliders (UsdPhysics.CollisionAPI, no RigidBody) at the config pose in `world`.
if args.obstacles:
    import math
    import yaml
    import omni.usd
    ...
    def _quat_rpy(rpy_deg):                                  # ROS rpy (R=Rz*Ry*Rx) -> Gf.Quatf
        r, p, y = [math.radians(a) for a in rpy_deg]
        cr, sr = math.cos(r/2), math.sin(r/2); cp, sp = math.cos(p/2), math.sin(p/2)
        cy, sy = math.cos(y/2), math.sin(y/2)
        return Gf.Quatf(cr*cp*cy + sr*sp*sy,                 # w
                        Gf.Vec3f(sr*cp*cy - cr*sp*sy, cr*sp*cy + sr*cp*sy, cr*cp*sy - sr*sp*cy))

    _ocfg_path = Path(args.obstacles).resolve()
    _ocfg = yaml.safe_load(_ocfg_path.read_text())
    _ometa = _ocfg.get("meta", {})
    _odefaults = _ometa.get("defaults", {})
    _oout = (_ocfg_path.parent / _ometa.get("out_usd_dir", "../../assets/obstacles")).resolve()
    _ostage = omni.usd.get_context().get_stage()
    UsdGeom.Scope.Define(_ostage, "/World/obstacles")
    for _o in _ocfg.get("obstacles", []):
        _oid = _o["id"]
        _coll = _o.get("collision", _odefaults.get("collision", "convex"))
        _xyz = [float(v) for v in _o["pose"]["xyz"]]
        _rpy = _o["pose"].get("rpy", [0, 0, 0])
        _path = f"/World/obstacles/{_oid}"
        if _coll == "box":
            _prim = UsdGeom.Cube.Define(_ostage, _path)
            _prim.GetSizeAttr().Set(1.0)
            _xf = UsdGeom.Xformable(_prim.GetPrim()); _xf.ClearXformOpOrder()
            _xf.AddTranslateOp().Set(Gf.Vec3d(*_xyz))
            _xf.AddOrientOp().Set(_quat_rpy(_rpy))
            _xf.AddScaleOp().Set(Gf.Vec3f(*[float(v) for v in _o["size"]]))
            _prim.GetDisplayColorAttr().Set(Vt.Vec3fArray([Gf.Vec3f(0.55, 0.55, 0.6)]))
            UsdPhysics.CollisionAPI.Apply(_prim.GetPrim())
        else:                                               # cad -> prepared USD
            _usd = _oout / f"{_oid}.usd"
            if not _usd.exists():
                carb.log_warn(f"obstacle '{_oid}': {_usd} missing (run prepare_obstacles.py); skipped")
                continue
            _prim = _ostage.DefinePrim(_path, "Xform")
            _prim.GetReferences().AddReference(str(_usd))
            _xf = UsdGeom.Xformable(_prim); _xf.ClearXformOpOrder()
            _xf.AddTranslateOp().Set(Gf.Vec3d(*_xyz))
            _xf.AddOrientOp().Set(_quat_rpy(_rpy))
            _mapi = UsdPhysics.MeshCollisionAPI.Apply(_prim)
            _mapi.CreateApproximationAttr().Set("convexDecomposition")
            UsdPhysics.CollisionAPI.Apply(_prim)
        print(f"  static obstacle     : {_path} [{_coll}] @ {_xyz}")
    simulation_app.update()
...
# ---- parallel-gripper mimic mirror (set 4) ---------------------------------
# A parallel gripper is a single DOF (driven finger + mimic). ros2_control commands
# ONLY the driven finger and Isaac's PhysxMimicJointAPI does not actually couple the
# pair, so the mimic finger would stay put in the sim (one jaw closes). Mirror the
# driven finger's position onto the mimic finger every step so BOTH jaws move —
# purely sim visual/grasp fidelity; MoveIt/RViz already enforce the single DOF via
# the URDF <mimic>. GUARD: engages only when both a *finger_left_joint and a
# *finger_right_joint exist, so this is a no-op for sets 1-3 (robotiq finger_joint).
_mirror = None
try:
    import numpy as _np
    from isaacsim.core.utils.types import ArticulationAction as _AA
    _dof = list(_art.dof_names)
    _drv = next((i for i, n in enumerate(_dof) if n.endswith("finger_left_joint")), None)
    _mim = next((i for i, n in enumerate(_dof) if n.endswith("finger_right_joint")), None)
    if _drv is not None and _mim is not None:
        _mirror = (_drv, _mim)
        print(f"  gripper mimic      : mirroring {_dof[_mim]} <- {_dof[_drv]} each step", flush=True)
except Exception as _e:
    carb.log_warn(f"gripper mimic mirror disabled (continuing): {_e}")

# OnPlaybackTick drives the graph every rendered step; just keep stepping.
while simulation_app.is_running():
    simulation_context.step(render=True)
    if _mirror is not None:                      # mirror driven->mimic finger target
        _q = _art.get_joint_positions()
        if _q is not None:
            _art.apply_action(_AA(joint_positions=_np.array([float(_q[_mirror[0]])]),
                                  joint_indices=_np.array([_mirror[1]])))
