...
### 8.10 V8 재판정 — 30 ep/태스크, **14/29 (ACT 씬) · 3/9 (8.9 씬)** (2026-09-15, `HISTORY.md` §46)

```
수집 90/90 (반경 0.06, ACT 기준선과 같은 씬) → h264 → 절대 10k, OMP_NUM_THREADS=8: 1 h 48 m, loss 0.014, 28.3 GB
A (0.06, 10/태스크)  red→left 8/10   blue→left 5/10   red→right 1/9    = 14/29   성공 배치 평균 22.8 mm
B (0.025, 3/태스크)  red→left 0/3    blue→left 1/3    red→right 2/3    = 3/9     (8.9 와 같은 숫자·같은 모양)
WRONG_OBJECT 0 · WRONG_PLACE 0 — 언어는 여전히 맞다. 합산 17/38 (45 %) vs 8.9 의 3/8 (38 %).
```
**데이터 4.3배로 성공률이 안 움직였다** → V8 의 "실패 시: 데이터 부족 판정"(§4)은 더 이상 성립하지 않는다.
실패 15건 중 14건이 **사전파지 높이까지 정확히 내려간 뒤 정지**(그리퍼 개방, 70 s 무동작). 데모의 최장 정지 0.8 s,
시작 자세와 무관 — 절대 액션 정책의 고정점으로 보인다(가설, §46.3). §3 의 상대 액션이 정공법이지만 서비스 불가(§8.8)라
**청크 단위 후처리 구현**이 필요한 "별도 구현" 칸이다.
**`num_inference_timesteps` 4→16 은 해 봤고 효과가 없다**(2026-09-16, `HISTORY.md` §46.7): 같은 시드 26회 대응에서 14→9,
성공↔실패가 양방향으로 뒤집힘(9↓ 4↑), 실패 종류·배치 오차 불변, 지연 80→160 ms. §4.2 의 "추론 전용 손잡이"는 지연용이지
정지 해결책이 아니다. 남은 것: 정지 관측 직접 검사 → 상대 액션 청크 후처리 구현.
