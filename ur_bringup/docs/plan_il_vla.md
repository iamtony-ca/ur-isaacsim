...
**해법: 임의 자세를 쫓지 말고 정해진 랑데부 자세에서 만난다.** 그리고 그 자세는 이미 있다 —
ROBOTIS OMY SRDF 의 `home`(= 손 떼도 서 있는 자세)이 우리 매핑을 통과하면 정확히 `ready` 가 된다:

```
leader home [0, 0, +90°, −90°, +90°, 0]  →  UR16e [0, −90°, +90°, −90°, −90°, 0]  =  reset_pose.py ready
```

우연이 아니라 양쪽 다 "팔꿈치 90° 접고 손목 정리"를 고른 결과다(우리는 특이점 회피,
ROBOTIS 는 내려놓기). **UR16e 쪽 초기 자세를 새로 정의할 필요가 없고**, IL 에피소드 리셋 자세와도
같아 **텔레옵 시작 자세 = 데이터 시작 자세**가 된다.
