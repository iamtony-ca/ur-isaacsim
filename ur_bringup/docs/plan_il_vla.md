...
> **문서 위치**: 이 문서가 **IL/VLA 정본**이며, 이 워크스페이스의 방향 그 자체다.
>
> **2026-09-08 문서 정리.** IL/VLA 로 전환하기 전 트랙의 설계문서 둘을 [`../../archive/`](../../archive/)
> 로 **보류 이동**했다(삭제 아님 — 사유와 복귀 시 주의사항은 그 폴더의 README).
> - `to_do.md` (foundation perception + pick&place 작업레이어) — 실현된 부분인 **M3 pick&place
>   상태머신은 `ur_bringup/scripts/pick_place_demo.py` 로 존재**하고 그 검증 이력은
>   `HISTORY.md` §25·§26 에 있다. 나머지(FoundationStereo/SAM3/FoundationPose, 동적 장애물 회피)는
>   IL/VLA 경로 밖이다.
> - `LEARNING.md` (RL insertion, Isaac Lab) — 현재 목표(teleoperation → ACT → GR00T N1.7)에 RL 은
>   없다. 살아있던 항목인 §2(컨트롤러 인터페이스 불일치)는 이미 이 문서 §2.3 으로 이관됐고,
>   `oht_bolting` 참조는 애초에 다른 워크스페이스 자산이었다.
>
> 그 두 문서에 기대던 서술은 이 문서 안에서 자립하도록 고쳤다.

> **2026-09-06 개정 요약** — 무엇이 바뀌었나
> 1. **데이터 생성 엔진 교체**: Isaac Lab Mimic → **cuMotion 상태머신 + 실물 freedrive** (§3).
>    상태머신은 `ur_bringup/scripts/pick_place_demo.py` 로 구현됨(HISTORY.md §25·§26).
> 2. **실물 freedrive 티칭을 앞으로**: leader/follower 없이 사람 궤적 확보 가능 (§1, §3)
> 3. **ROS2 경계 재정의**: 로봇 I/O·기록·안전은 기존 ROS2 스택 재사용 (§2.3).
>    ~~정책 추론만 소켓 밖~~ → **2026-09-08 정정**: lerobot 정책은 소켓 없이 ML venv 에서
>    상류 `async_inference` 를 그대로 쓴다(§2.2 의 정정 박스). 소켓은 lerobot 밖 스택에만.
...
| 주변 인프라(캘리브레이션, nvblox, perception)는? | **예** (이미 ROS 2 기반) |
...
구 작업레이어 계획은 v1 에서 **정적 카메라 1대로 장애물+타깃을 모두** 처리하고 손목 카메라를 v2 로 미뤘다.
...
| 3 | **GR00T N1.7** (3B, **동결 파인튜닝** — LoRA 아님) | 최종 목표. 언어 일반화 + 사전학습 전이 |

> **★ 실행 설계는 별도 문서로 분리했다 — [`plan_groot_n17.md`](plan_groot_n17.md)** (2026-09-10).
> 재사용 경계표, 카메라 2대 연결 근거, 절대/상대 액션 결정, 검증 계획과 합격 기준, 실측 결과.
> 아래 §2.7/§6.2 중 GR00T 관련 서술은 그 문서가 정정한다.

§2.8 이 설계한 **ACT vs VLA 동일조건 비교**가 2단계에서 이미 가능해진다 — 실험을 앞당기고 비용을 줄인다.

> **★ 2026-09-13 결정 — 2단계(소형 VLA)는 생략한다.** 이 칸을 넣은 이유는 "3B 가 32 GB 에 안 들어갈
> 도박의 결과를 E단계에 가서야 알게 된다"였는데, GR00T N1.7 이 `fp32=false` + batch 32 로 27.3 GB 에서
> 학습되고(3.7 h/10k 스텝) sim 롤아웃까지 닫혔다(`plan_groot_n17.md` §8, `HISTORY.md` §44). 도박이
> 아니게 됐으므로 중간 단계의 근거가 사라졌다. 언어 조건 확인은 GR00T 에서 직접 한다(`WRONG_OBJECT` 0/8 —
> 재수집 30 ep/태스크 후 재판정 2026-09-15: `WRONG_OBJECT` 0/38 유지, 성공 14/29 — 실패는 언어가 아니라 사전파지 정지,
> `HISTORY.md` §46). 사다리는 **ACT → GR00T** 두 단으로 읽는다.
...
> **★ 2026-09-10 실측 — 위 낙관은 절반만 맞았다** (`HISTORY.md` §40.5, `plan_groot_n17.md` §8.2)
>
> 동결은 의도대로 걸린다. 그런데 **"projector + diffusion head 만"이 가볍지 않다** —
> action head 가 **1,621 M** 으로 동결된 백본(1,524 M)보다 크다. 전체 3,144 M 중 **51.5% 가 학습 대상**이다.
> ```
> model_params_fp32=True (기본):  params 11.7 + grads 6.0 + AdamW 12.1 = 29.8 GiB / 31.8 GiB
> ```
> **배치 크기와 무관하게** 정적분만으로 카드가 찬다. → **`--policy.model_params_fp32=false`**
> (action head 만 bf16) 로 **≈17.8 GiB**. upstream 손잡이이므로 우회가 아니다.
>
> **그리고 "LoRA" 는 오기다.** `configuration_groot.py` 의 `lora_*` 필드는 소스 주석이
> **"never-wired"** 라고 명시한 deprecated 필드다. 메모리 관리는 LoRA 가 아니라
> **동결 + 파라미터 dtype** 으로 한다.
...
- 이렇게 모으면 동일 데이터로 ACT(언어 무시) vs GR00T(언어 사용)를 비교 가능 — 이 비교가
  프로젝트를 튜토리얼 실행이 아닌 실험으로 만든다

**★ 단, "같은 데이터셋을 그대로 ACT 에 먹인다"는 뜻이 아니다(2026-09-09 실수).**
lerobot 0.6.1 `policies/act/` 에는 토크나이저가 없고 `forward()` 는 `OBS_IMAGES`+`OBS_STATE`
만 받는다. 우리 씬은 **모든 물체·마커가 매 에피소드에 존재**하므로, 3종을 섞은 데이터셋은
ACT 에게 "같은 픽셀, 다른 정답" 이 된다 — 언어를 무시하는 정도가 아니라 **모순된 지도학습**이라
평균으로 수렴한다. 그러면 그 ACT 는 "언어 없는 정책의 성능" 이 아니라 그냥 망가진 실행이고,
비교의 기준선 역할을 못 한다.

| 단계 | 데이터셋 | 체크포인트 |
|---|---|---|
| **ACT** | 태스크 **1종**만 필터해서 사용 | 태스크마다 따로 |
| **소형 VLA / GR00T** | 3종 **합친 그대로** | 하나 |

수집은 3종을 한 디렉터리에 모으는 지금 방식을 유지한다 — `task` 문자열이 메타에 남으므로
ACT 용 단일 태스크 데이터셋은 **필터로 만들면 되고, 재수집이 필요 없다.**
공정한 비교는 "같은 태스크에서 ACT(1종 학습) vs VLA(3종 학습)" 이다.
...
UR16e 가 다른 작업을 하다 텔레옵으로 넘어오면 리더와 시작 자세가 크게 다르다. 현재
`/omy_bridge/enable` 은 **거부**하고(관절별 오차를 알려준다), 조작자가 리더를 손으로 맞추는
경로 하나뿐이다. 큰 차이에는 실용적이지 않다.

**해법: 임의 자세를 쫓지 말고 정해진 랑데부 자세에서 만난다.** 그 자세는 리더가 실제로 놓이는 자세,
즉 ROBOTIS 브링업이 OMY 팔로워를 보내는 `ready` 이고, 우리 매핑을 통과한 값이 UR16e `ready` 다
(2026-09-16 재정의, `HISTORY.md` §47):

```
leader ready [0, −90°, +152°, −62°, +90°, 0]  →  UR16e [0, −180°, +152°, −152°, −90°, 0]  =  reset_pose.py ready
```

> 2026-09-10 판은 SRDF `home`(상완 수직)을 리더 휴식 자세로 잘못 보고 [0, −90°, +90°, −90°, −90°, 0] 을
> `ready` 로 잡았다(`HISTORY.md` §41). 실물에서 리더는 거기 놓이지 않아 첫 동작이 튀었다. 구 자세는
> `ready_v1` 로 남긴다(그 전 데이터셋의 시작 자세). J4 오프셋 −90° 는 J2 처럼 유도된다(§47.2).

**UR16e 초기 자세 = 랑데부 = IL 에피소드 리셋 자세**(`pick_place_demo.py` READY)라
**텔레옵 시작 자세 = 데이터 시작 자세**가 된다.

- ⚠️ L100 의 `init`(전부 0)을 쓰면 UR16e 는 팔꿈치 특이점(`home`)이 된다. **랑데부는 UR16e 쪽
  제약(특이점·테이블·픽스처) 기준으로 정한다.**
- ⚠️ **리더는 자동으로 초기 자세에 가지 않는다** — `omy_l100_leader_ai.launch.py` 는 중력보상
  컨트롤러만 스폰하고 `init_position`/`arm_controller` 가 없다(그건 팔로워 런치에만 붙는다).
  "쉽게 간다"는 자동 구동이 아니라 **중력보상 덕에 사람이 쉽게 든다**는 뜻이다.
**구현됨 (2026-09-10, mock + Isaac sim 검증 완료 — `HISTORY.md` §42)**:

| | |
|---|---|
| `/omy_bridge/sync` | MoveIt 으로 UR16e 를 `rendezvous`(기본 `ready`)로. 컨트롤러 전환까지 처리하고 **streaming 으로 되돌려 둔다**. 즉시 반환(진행상황은 `/omy_bridge/status`) |
| `/omy_bridge/engage_error` | `Float64MultiArray` 6개, 5 Hz. enable 을 호출해 거부당하지 않고도 리더를 맞출 수 있다 |
| 패드 바인딩 | `pad:=true`. Options=enable / R3=sync (**데드맨 필요**) / Create=disable (불필요) |
| enable 가드 | streaming 컨트롤러가 비활성이면 거부 — "engage 됐는데 안 움직임"이라는 무증상 실패 방지 |

아직 없는 것: **리더를 자동으로 랑데부로 구동**(중력보상→position 전환이 필요하고, 사람이
잡고 있는 팔에 그걸 하는 건 그 자체가 안전 사건 — 실물 확인 후 판단).

상세와 실물 측정 항목: `HISTORY.md` §41·§42, `CHECKLIST.md` E-1~E-3.
...
| **B′** | ~~Isaac Lab 포팅~~ → **cuMotion pick&place 상태머신** (perception 은 **뺌**) | 단축됨 | pick&place 상태머신. **IL 데이터 엔진 겸함**. pose 는 Isaac GT `/scene/object_pose` |
...
- **B 는 크게 줄었다** — 2F-85 물리는 **이미 완료**되어 있고(§5), instanceable 변환도 불필요해졌다.
  대신 pick&place 상태머신 구축이 그 자리에 들어간다(어차피 할 일이었다).
...
- **GR00T (3B) 전체 파인튜닝**: 어려움. NVIDIA 문서가 최적 성능 기준 H100/L40 노드를 권장, LoRA는 A6000 2장 또는 RTX 4090 2장(=48GB)을 사용했다고 명시. 32GB 단일 카드는 그 아래
  → ~~LoRA + gradient checkpointing + 배치 축소~~ **(2026-09-10 정정)** lerobot 0.6.1 의 GR00T N1.7
  경로에 **LoRA 는 없다**(`lora_*` = never-wired deprecated 필드). 실제 손잡이는 **동결 범위 +
  파라미터 dtype**:
  - **실측**: 전체 3,144 M / **학습 대상 1,621 M(51.5%)** — 백본(1,524 M)은 전부 동결되는데도
    action head 가 그보다 크다
  - `model_params_fp32=True`(기본): **정적 29.8 GiB / 31.8 GiB** → 활성값 자리 없음, **배치 무관 실패**
  - **`--policy.model_params_fp32=false` → ≈17.8 GiB** ← 이걸 기본으로 쓴다
  - 상세: [`plan_groot_n17.md`](plan_groot_n17.md) §8.2 / `HISTORY.md` §40.5
- **추론**: 문제없음. 단일 샘플 처리에서 최신 GPU 간 차이 작고, denoising step 4면 충분
  (미측정 — 청크 40 @30 Hz = **1.33 s 안에 추론이 끝나야** 로봇이 멈칫하지 않는다.
  넘치면 `num_inference_timesteps` 로 낮춘다 — 재학습 불필요한 추론 전용 손잡이)
...
- **라이선스 확인** — GR00T / openpi / 소형 VLA 각각의 상업 사용 조건.
  SAM3·ultralytics(AGPL) 라이선스는 perception 을 실제로 도입할 때 착수 전 확인.
...
5. **태스크 3종 정의** — 언어 지시문 포함, §2.8 참조. (2026-09-08 완료: red/blue × left/right 3종)
6. **pick&place 상태머신 완성** — 완료 시점이 곧 C′ 의 데이터 엔진 완성 시점. (2026-09-08 완료)
